...
if (
    !$this->module &&
    isset($this->element->category_canonical) &&
...
if (
    ...
    hikaInput::get()->getVar("task") == "listing" &&
...
function setToCompareList(product_id,name,elem) {
	...
	if(compare_list[product_id]) {
		...
		if(compare_list.length < <?php echo (int) $this->config->get(
      "compare_limit",
      5
  ); ?> ) {
...
function compareProducts() {
	...
	window.location = "<?php
 $u = hikashop_completeLink(
     "product&task=compare" . $this->itemid,
     false,
     true
 );
 if (strpos($u, "?") === false) {
...
window.Oby.registerAjax('compare.updated', function(evt){
	...
	if(evt.added && (evt.size == 0 || evt.size < <?php echo (int) $this->config->get(
     "compare_limit",
     5
 ); ?>))
		return true;
	if(!evt.added && evt.size >= <?php echo (int) $this->config->get(
     "compare_limit",
     5
 ); ?>)
		return true;
...
<!-- TOP EXTRA DATA -->
<?php if (!empty($this->element->extraData->top)) {
...
if (
    !empty($title) &&
    hikaInput::get()->getVar("hikashop_front_end_main", 0) &&
    (!$this->module || $this->pageInfo->elements->total)
) {
...
if (
    ($this->params->get("show_image") && !empty($this->element->file_path)) ||
    ($this->params->get("show_description", !$this->module) &&
        !empty($this->element->category_description))
) { ?>
...
if ($this->params->get("show_image") && !empty($this->element->file_path)) {
    ...
    if (
        JFile::exists($this->image->getPath($this->element->file_path, false))
    ) { ?>
			<img src="<?php echo $this->image->getPath(
       $this->element->file_path
   ); ?>" class="hikashop_category_image" title="<?php echo $this->escape(
    (string) @$this->element->file_description
); ?>" alt="<?php echo $this->escape((string) @$this->element->file_name); ?>"/>
<?php }
}
if (
    $this->params->get("show_description", !$this->module) &&
    !empty($this->element->category_description)
			...
			<div class="hikashop_category_description_content"><?php echo JHTML::_(
       "content.prepare",
			...
			<tr class="hikashop_category_custom_<?php echo $oneExtraField->field_namekey; ?>_line">
				...
				<td>
					<span id="hikashop_category_custom_value_<?php echo $oneExtraField->field_id; ?>" class="hikashop_category_custom_value">
						<?php echo $this->fieldsClass->show(
							$oneExtraField,
...
<!-- CATEGORY FIELDS -->
<!-- MIDDLE EXTRA DATA -->
<?php if (!empty($this->element->extraData->middle)) {
...
if (
    ...
    (hikaInput::get()->getVar("task", "listing") == "listing" ||
        !empty($this->force_display_filter)) &&
    $display_filters == 1
) {
...
$classes .= HIKASHOP_J40 ? ' hika_j4' : ' hika_j3';
...
if (!$this->module || hikaInput::get()->getVar("hikashop_front_end_main", 0)) {
    ...
    $attributes =
        'data-refresh-class="hikashop_checkout_loading" data-refresh-url="' .
        $url .
        '" data-use-url="1"';
}
...
if (
    ...
    $css_button_compare = $this->config->get(
        "css_button_compare",
        "hikabtn-compare"
    );
    ?>
			<div id="hikashop_compare_zone" class="hikashop_compare_zone">
				<a class="<?php echo $css_button .
        " " .
        $css_button_compare; ?>" id="hikashop_compare_button" style="display:none;" href="#" data-compare-href="<?php echo hikashop_completeLink(
    "product&task=compare" . $this->itemid,
    false,
    true
); ?>" onclick="if(window.hikashop.compareProducts) { return window.hikashop.compareProducts(this); }"><span><?php echo JText::_(
    "COMPARE_PRODUCTS"
); ?></span></a>
			</div>
<?php
...
<?php
        ...
        echo '<div class="hk-well hika_no_products"><i class="fa fa-search"></i> ' .
            JText::_("HIKASHOP_NO_RESULT") .
            "</div>";
    }
} elseif (!empty($this->rows) && !empty($this->categories)) {
    if (!$this->module) {
        echo $mainInfo;
    }
    ...
    if (
        (!empty($allrows) ||
            !$this->module ||
            hikaInput::get()->getVar("hikashop_front_end_main", 0)) &&
        in_array($pagination, ["top", "both"]) &&
        $this->params->get("show_limit") &&
        $this->pageInfo->elements->total
    ) {
        $this->pagination->form = "_top"; ?>
	<form action="<?php echo str_replace(
     ["&tmpl=raw", "&tmpl=component"],
     "",
     hikashop_currentURL()
 ); ?>" method="post" name="adminForm_<?php echo $this->params->get(
    "main_div_name"
) . $this->category_selected; ?>_top">
      ...
      $this->pagination->getListFooter($this->params->get("limit"))
		...
		<input type="hidden" name="filter_order_<?php echo $this->params->get(
      "main_div_name"
  ) . $this->category_selected; ?>" value="<?php echo $this->pageInfo->filter
    ->order->value; ?>" />
		<input type="hidden" name="filter_order_Dir_<?php echo $this->params->get(
      "main_div_name"
  ) . $this->category_selected; ?>" value="<?php echo $this->pageInfo->filter
    ->order->dir; ?>" />
...
<?php
    ...
    foreach ($this->categories as $category) {
        ...
        $this->params->set(
            "main_div_name",
            $main_div_name . "_" . $category["category"]->category_id
        );
	...
	<div id="<?php echo $main_div_name .
     "_" .
     $category["category"]->category_id; ?>" class="hikashop_products_listing">
<?php
if (
    hikaInput::get()->getVar("hikashop_front_end_main", 0) &&
    hikaInput::get()->getVar("task") == "listing" &&
...
<?php
    ...
    if (
        (!empty($allrows) ||
            !$this->module ||
            hikaInput::get()->getVar("hikashop_front_end_main", 0)) &&
        in_array($pagination, ["bottom", "both"]) &&
        $this->params->get("show_limit") &&
        $this->pageInfo->elements->total
    ) {
    ...
    "main_div_name"
) . $this->category_selected; ?>_bottom">
...
?>
<!-- BOTTOM EXTRA DATA -->
<?php if (!empty($this->element->extraData->bottom)) {
...
if (
	...
	<div id="<?php echo $this->params->get(
     "main_div_name"
 ); ?>" class="<?php echo $classes; ?>" <?php echo $attributes; ?>>
<?php
